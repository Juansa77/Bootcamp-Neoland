...
const mostGamer = streamers.filter(
...
const streamWithU = streamersTwo.filter((element) =>
  element.name.includes("u")
);
...
const streamerLegend = streamersTwo.filter((element) => {
...
const finder = numbers.find((element) => element > 99);
console.log(finder);
...
const finderMovie = movies.find(({ date }) => date == 2010);
console.log(finderMovie);
...
const finderAlien = aliens.find(({ name }) => name == "Cucushumushu");
const finderMutation = mutations.find(({ name }) => name == "Porompompero");
const sumArrayS = { ...finderAlien, mutations: finderMutation.name };
...
const sum = exams.reduce(function (acuu, curr) {
  return acuu + curr.score;
}, valorInicial);
console.log(sum);
...
for (const key of exams) {
  if (key.score >= 5) aprobados.push(key.score);
}

for (const iterator of exams) {
  todos.push(iterator.score);
}
var sumaAprobador = aprobados.reduce(
  (acumulator, current) => acumulator + current
);
console.log(sumaAprobador);

/*7.3 Dado el mismo array, haz la media de las notas de todos los examenes .reduce().*/
const media = todos.reduce((acuu, curr, media) => {
